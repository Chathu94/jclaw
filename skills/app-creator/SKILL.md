...
1. **Build** the app inside your workspace, in a directory named for the slug: `<slug>/index.html`, `<slug>/app.json`, and a generated `<slug>/icon.svg`.
...
Pin down what the app does (its pages, the core action) and a display name. Derive `<slug>` = the name lowercased, spaces → hyphens, non-alphanumerics stripped (e.g. "Proposal Generator" → `proposal-generator`); it must match `^[a-z0-9][a-z0-9-]*$`. Confirm it if ambiguous. Capture: **name**, **creator** (default the operator), **version** (default `1.0.0`), an optional **price/plan** label (metadata only), and an optional **designated agent id** (JCLAW-763 — the one agent the app may invoke; passed by the Create/Update form). An **`icon.svg` is generated by default** from the app's description (see §2/§3) — no operator input needed.
...
**Build posture — harness first, always.** The ACP coding harness (`runtime=acp`) is the **default** builder, not a preference: whenever a harness is available you MUST delegate the whole build to it. Building it yourself with your own `exec`/`filesystem` tools is a **fallback used only when a `runtime=acp` spawn reports that no harness is configured** — never build directly while a harness is available. (Either app shape.) Either way, the app is built into your workspace `<slug>/`, never into `public/apps/`.

### Default — delegate to the ACP coding harness (runtime=acp)
...
> Create a `<slug>/` directory in your current working directory and write **exactly** these three files into it, and nothing else: `<slug>/index.html`, `<slug>/app.json`, and `<slug>/icon.svg`.
> `<slug>/icon.svg` is a small square app icon you design from the app's purpose — a single clear glyph or shape relevant to what the app does, on a solid or subtle-gradient background, with a `viewBox` (e.g. `0 0 64 64`). Keep it simple and self-contained (no external references).
> `<slug>/app.json` must be EXACTLY this object — **these keys only, and no others** (do NOT add `slug`, `entry`, or any extra key):
> ```json
> {"name": "<Display Name>", "version": "1.0.0", "creator": "<creator>", "description": "<one-sentence summary>", "icon": "icon.svg", "price": "", "agent": <id>}
> ```
> Leave `price` as `""` for a free app. Set `agent` to the designated agent id when one is given, otherwise **omit the `agent` key entirely**.
...
`runtime=acp` runs the operator-configured harness (`subagent.acp.command`). **Only if a `runtime=acp` spawn reports that no harness is configured** do you fall through to the direct path below — and do NOT ask the operator to configure one unless they want the harness.

### Fallback — build it yourself (ONLY when no harness is available)

Reach this path **only** after a `runtime=acp` spawn reported no harness — otherwise you must delegate (see the posture above). Build directly with your `exec` + `filesystem` tools, writing into your workspace `<slug>/`. Self-contained static: author `<slug>/index.html` (inline CSS/JS, client-side storage) **and** `<slug>/icon.svg` (a simple app icon derived from the description — same guidance as the harness task above). Nuxt shape: scaffold a lean Nuxt 4 app in a workspace build dir, set `ssr: false` + `app.baseURL: '/apps/<slug>/'`, `npx nuxi generate`, place `.output/public/*` into `<slug>/`, plus an `icon.svg`. Either way, write the manifest with `"icon": "icon.svg"`, then install (section 4).
...
- `icon` is a path **relative to the app dir**. **Generate an `icon.svg` by default** — a simple square SVG icon derived from the app's description (a relevant glyph/shape on a solid or subtle-gradient background) — and set `"icon": "icon.svg"`. Only omit it (and drop the file) if the operator explicitly wants no icon; then the Apps page shows its default tile.
...
2. **Apply the changes** in your workspace `<slug>/`, keeping the SAME shape it already uses (a self-contained `index.html`, or a Nuxt rebuild with the same `app.baseURL: '/apps/<slug>/'`). Preserve the slug, name, creator, price, and `icon.svg` unless the operator asked to change them. If the request sets, changes, or removes the **designated agent**, write (or delete) the `agent` field in `<slug>/app.json` to match the named id. Delegate the edits to the harness (`runtime=acp`) with the staged files as context — the same harness-first posture as a new build; edit directly only when no harness is available.
