...
# JClaw skill layout is `SKILL.md + credentials/ + tools/`, and this script lives in
# `tools/`, so cookies sit one level up in `credentials/`. Both paths are env-overridable
# so the script still runs standalone outside a skill directory.
SKILL_ROOT = os.path.dirname(PATH)
COOKIES_FILE = os.environ.get("SAFARIBOOKS_COOKIES") or os.path.join(SKILL_ROOT, "credentials", "cookies.json")
OUTPUT_DIR = os.environ.get("SAFARIBOOKS_OUTPUT") or os.path.join(PATH, "Books")
...
class SafariBooks:
    ...
    API_TEMPLATE_V1 = SAFARI_BASE_URL + "/api/v1/book/{0}/"
    API_TEMPLATE_V2 = SAFARI_BASE_URL + "/api/v2/epubs/urn:orm:book:{0}/"
    API_TEMPLATE = API_TEMPLATE_V2  # v1 is deprecated (returns 404 since ~2026)
    ...
    def get_book_info(self):
        ...
        # v2 API compatibility: map v2 fields to v1 field names used throughout the code
        if "ourn" in response:
            response = self._map_v2_book_info(response)
    ...
    def _map_v2_book_info(self, v2):
        """Map v2 API response fields to v1 format expected by the rest of the code."""
        desc_html = ""
        if isinstance(v2.get("descriptions"), dict):
            desc_html = v2["descriptions"].get("text/plain", v2["descriptions"].get("text/html", ""))

        # Build v1-compatible response
        mapped = {
            "identifier": v2.get("identifier", ""),
            "isbn": v2.get("isbn", v2.get("identifier", "")),
            "title": v2.get("title", ""),
            "description": desc_html,
            "web_url": SAFARI_BASE_URL + "/library/view/-/" + v2.get("identifier", "") + "/",
            "issued": v2.get("publication_date", ""),
            "rights": "",
            "virtual_pages": v2.get("virtual_pages", 0),
            "page_count": v2.get("page_count", 0),
            "content_format": v2.get("content_format", "book"),
            "language": v2.get("language", "en"),
            "roughcut": v2.get("roughcut", False),
            # v2 stores authors/publishers/subjects differently — these URLs need following
            # For now, use empty lists (the code handles missing gracefully)
            "authors": [],
            "publishers": [],
            "subjects": [],
        }

        # Preserve v2-specific fields for chapter fetching
        mapped["_v2_chapters_url"] = v2.get("chapters", "")
        mapped["_v2_toc_url"] = v2.get("table_of_contents", "")
        mapped["_v2_files_url"] = v2.get("files", "")
        mapped["_v2_spine_url"] = v2.get("spine", "")

        return mapped

    def get_book_chapters(self, page=1):
        # v2 API: use the chapters URL from book info if available
        chapters_url = self.book_info.get("_v2_chapters_url", "")
        if chapters_url:
            return self._get_book_chapters_v2(chapters_url)
    ...
    def _get_book_chapters_v2(self, chapters_url):
        """Fetch chapters from v2 API and map to v1 format expected by get()."""
        all_chapters = []
        url = chapters_url
        files_base = SAFARI_BASE_URL + "/api/v2/epubs/urn:orm:book:{}/files".format(self.book_id)

        while url:
            response = self.requests_provider(url)
            if response == 0:
                self.display.exit("API: unable to retrieve book chapters (v2).")

            data = response.json()
            if not isinstance(data, dict):
                self.display.exit("API: unexpected v2 chapters response.")

            for ch in data.get("results", []):
                # Extract filename from reference_id: "9781835083468-/Text/cover.xhtml" → "Text/cover.xhtml"
                ref_id = ch.get("reference_id", "")
                filename = ref_id.split("-", 1)[-1].lstrip("/") if "-" in ref_id else ref_id
                if not filename:
                    ourn = ch.get("ourn", "")
                    if "chapter:" in ourn:
                        filename = requests.utils.unquote(ourn.split("chapter:")[-1])

                # Fetch full chapter detail (has related_assets with images, stylesheets, content_url)
                ch_detail_url = ch.get("url", "")
                images = []
                stylesheets = []
                content_url = ch.get("content_url", "")

                if ch_detail_url:
                    ch_detail = self.requests_provider(ch_detail_url)
                    if ch_detail != 0:
                        ch_data = ch_detail.json()
                        content_url = ch_data.get("content_url", content_url)
                        related = ch_data.get("related_assets", {})

                        # Extract image URLs
                        for img_url in related.get("images", []):
                            images.append(img_url)

                        # Extract stylesheet URLs
                        for css_url in related.get("stylesheets", []):
                            stylesheets.append({"url": css_url})

                # Build v1-compatible chapter dict expected by get()
                all_chapters.append({
                    "filename": filename,
                    "title": ch.get("title", filename),
                    "content": content_url,  # get() fetches HTML from this URL
                    "asset_base_url": files_base,
                    "images": [img.replace(files_base + "/", "") for img in images],
                    "stylesheets": stylesheets,
                    "site_styles": [],
                    "full_path": filename,
                    "natural_key": [self.book_id, filename],
                })

            url = data.get("next")

        if not all_chapters:
            self.display.exit("API: no chapters found (v2).")

        # Sort: covers first
        covers = [c for c in all_chapters if "cover" in c["filename"].lower() or "cover" in c["title"].lower()]
        others = [c for c in all_chapters if c not in covers]
    ...
    def save_page_html(self, contents):
        ...
        output_path = os.path.join(self.BOOK_PATH, "OEBPS", self.filename)
        # Ensure parent directories exist (v2 API uses subdirs like Text/cover.xhtml)
    ...
    def _map_v2_toc(self, toc_list):
        """Map v2 TOC entries to v1 format expected by parse_toc."""
        mapped = []
        for entry in toc_list:
            ref_id = entry.get("reference_id", "")
            # Extract filename: "9781835083468-/Text/Title_Page.xhtml" → "Text/Title_Page.xhtml"
            href = ref_id.split("-", 1)[-1].lstrip("/") if "-" in ref_id else ref_id

            mapped_entry = {
                "depth": entry.get("depth", 1),
                "id": entry.get("ourn", ref_id),
                "fragment": entry.get("fragment", ""),
                "label": entry.get("title", ""),
                "href": href,
                "children": self._map_v2_toc(entry.get("children", [])) if entry.get("children") else [],
            }
    ...
    def create_toc(self):
        # v2 API: use the TOC URL from book info if available
        toc_url = self.book_info.get("_v2_toc_url", "")
        if toc_url:
        ...
        # v2 TOC returns a paginated dict with "results", v1 returns a list
        if isinstance(response, dict) and "results" in response:
            toc_list = response["results"]
            # Fetch all pages if paginated
            while response.get("next"):
                response = self.requests_provider(response["next"])
                if response == 0:
                    break
                response = response.json()
                toc_list.extend(response.get("results", []))
            response = toc_list
        ...
        # v2 TOC uses different field names — map to v1 format
        if response and isinstance(response[0], dict) and "ourn" in response[0]:
            response = self._map_v2_toc(response)
    ...
    def create_epub(self):
        ...
        zip_file = os.path.join(OUTPUT_DIR, self.book_id)
