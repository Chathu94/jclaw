...
| `/prompt`         | Search your [Prompts Library](/guide#prompts): `/prompt code review`. In the web composer the prompt's text drops straight into the box; on other channels JClaw replies with it so you can copy and edit it. `/prompt` on its own lists what you have saved. |
