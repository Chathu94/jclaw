---
name: sonar
description: Triage and fix SonarQube issues, security hotspots (blocker→info), AND excessive code duplication (>60% file density) for abundent:jclaw — fix legitimate ones in an isolated worktree, mark false positives/accepts/safe with justifications, and validate via ./jclaw.sh test on the worktree's own seeded test port.
category: Quality
...
7. Create a dedicated worktree off `main` so the fixes never touch the primary working tree:
   ...
   ```bash
   grep '^PLAY_TEST_PORT=' ../jclaw-sonar/certs/.env
   ```
   **Do not override it with a fixed port.** The primary tree pins its own value, so any hardcoded pin here risks colliding with a `play autotest` running there — which surfaces as a broad batch of unrelated controller failures, not as an obvious port error.
...
**Phase 4 — Validate on the worktree's own test port**

10. From the worktree, run the suite isolated on its seeded `PLAY_TEST_PORT`:
   ```bash
   ...
   - **Env-flake guard:** if a broad batch of *unrelated* controller/functional tests fails (401s, FK violations, search-scope mismatches), that's the known live-app/parallel-worktree interference — confirm the worktree's `PLAY_TEST_PORT` is the hook-seeded one and is actually free (`lsof -nP -iTCP:<port> -sTCP:LISTEN`), then re-run; do not chase it as a real failure.
